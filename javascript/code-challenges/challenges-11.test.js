...
------------------------------------------------------------------------------------------------ */
// got help from stack mozilla web docs
function transformToLis(obj){
  // Solution code here...
  let banana = []
  for (const [key, value] of Object.entries(obj)){

    banana.push((`<li>${key}: ${value}</li>`))
...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((taco, banana)=>taco=[...taco,...banana],[]).reduce((I,numba)=>(numba==target)?I=I+1:I,0);
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr=>arr.filter(potato=>typeof potato === 'number'&&potato % 5 === 0).map(banana=>2**banana));
};
...
let findShortest = (data) => {
  ...
  return data.reduce((length, index)=>length.height<index.height?length=index:length,{height:0}).name
