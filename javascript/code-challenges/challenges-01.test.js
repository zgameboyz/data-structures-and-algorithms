...
const addExclamation = (arr) => {
  // Solution code here...
  let test = [];
  arr.forEach( exclamation => test.push(exclamation + "!"));
  return test;
};
...
const allUpperCase = (arr) => {
  // Solution code here...
  let test = [];
  arr.forEach( upper => test.push(upper.toUpperCase()));
  return test;
};
...
const speaker = (words, callback) => {
  // Solution code here...
  let answers=[];
  words.forEach(words => answers.push(callback(words)));
  return answers;
};
...
const addNumbers = (num, arr, times, callback) => {
  ...
  return array;
  // arr.forEach(times => arr.push(callback(times)));
};
// 8, [], 5, addValues
